...
public class StateTracker
{
   ...
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "beginCycle");
      ...
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "beginCycle");
   }
   
   public void addState(String p_name, String p_parent, Object p_value)
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "addState");
      ...
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "addState");
   }
   
   public void endCycle()
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "endCycle");
      ...
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "endCycle");
   }
   
   public void addIOInfo(String p_name, String p_type, String p_direction, Object p_port)
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "addIOInfo");
      
      m_ioSet.addIOInfo(p_name, p_type, p_direction, p_port);
      
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "addIOInfo");
   }
   ...
   public ArrayList<StateGroup> getStateList()
   {
      ...
      // This avoids write clashes when read from the logger possibly on a separate thread
      synchronized (m_stateListLock)
      {
