...
public class Config
{
    ...
    * of the line that comes before that character, which will be an empty string if
    * the line starts with a '#'.  If a null string is passed in, a NullPointerException 
    * is thrown.  The String is trimmed before being returned to remove any whitespace on
    * either end
    * 
    * @param p_line a String representing a line of text
    * 
    * @return the String, minus any present # character, and any characters that follow it, if any. May
    * return an empty String, but will never return null
    * 
    * @throws NullPointerException if the input String is null
    */
   protected String stripComments(String p_line)
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "stripComments");
      ...
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "stripComments");

      return result;
   }
   
   /**
    * Splits the input String on an equals (=) character, and returns the two parts as an 
    ...
    * @return
    * 
    * @throws NullPointerException if the String passed in is null
    */
   protected String[] getKeyValuePair(String p_line)
   {
      String[] result;

      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "getKeyValuePair");

      result = p_line.split("=");
      
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "getKeyValuePair");

      return result;
   }

   /**
    * Parses the value from a String.  It attempts to parse it into a primitive type,
    * returns as its wrapper class.  The order it attempts is: double, int, boolean, String
    * @param p_valueStr
    * @return
    */
   ...
   protected Double parseDouble(String p_valueStr)
   {
      ...
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "parseDouble");
   ...
   protected Integer parseInt(String p_valueStr)
   {
      ...
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "parseInt");
   ...
   protected Boolean parseBoolean(String p_valueStr)
   {
      ...
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "parseBoolean");
   ...
   public Object getValue(String p_key, Object p_default)
   {
      ...
      if (s_log.isLoggable(Level.FINER)) s_log.exiting(s_className, "getValue");
   ...
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "getDouble");

      Object value = getValue(p_key, p_default);
      if (!(value instanceof Double))
      {
         throw new NumberFormatException("Value found for " + p_key + " but value was not a Double: " + value.getClass().getName());
      }
   ...
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "getInt");

      Object value = getValue(p_key, p_default);
      if (!(value instanceof Integer))
      {
         throw new NumberFormatException("Value found for " + p_key + " but value was not a Integer: " + value.getClass().getName());
      }
   ...
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "getBoolean");

      Object value = getValue(p_key, p_default);
      if (!(value instanceof Boolean))
      {
         throw new NumberFormatException("Value found for " + p_key + " but value was not a Boolean: " + value.getClass().getName());
      }
   ...
   {
      if (s_log.isLoggable(Level.FINER)) s_log.entering(s_className, "getString");

      Object value = getValue(p_key, p_default);
      if (!(value instanceof String))
      {
         throw new NumberFormatException("Value found for " + p_key + " but value was not a String: " + value.getClass().getName());
      }
      return (String)value;
   }
